...
def best_first_graph_search(problem, f):
    ...
    while frontier:
        ...
        for i in range(len(frontier)):
            frontierS = list(set(map(tuple, frontier.A)))
        print("Frontier")
        print(frontierS)
        print("")
    return None
...
def USMapHeuristic(node):
    return usa_map.locations[node.state]
...
usa_map.locations = dict(
    LosAngeles=1100, SanFrancisco=1230, Austin=182,
    Charlotte=929, NewYork=1368, Chicago=800,
    Seattle=1670, SantaFe=560, Bakersville=1282,
    Boston=1551, Dallas=0)

usa_problem = GraphProblem('Seattle', 'Dallas', usa_map)
r = recursive_best_first_search(usa_problem,USMapHeuristic)
print("")
astar_search(usa_problem,USMapHeuristic)
print("")

def isConsistent(usa_map, USMapHeuristic):
    consistent = True
    print("garbage here")
    for node in usa_map.nodes():
        print(node)
        for child, value in usa_map.get(node):
            print(child)
            print(value)
    return consistent
#isConsistent(usa_map, USMapHeuristic)
